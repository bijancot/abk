...
class AssignmentController extends CI_Controller {
    ...
    public function wsdetail($npm, $wsid, $tipe){
        ...
        $data['wsid']           = $wsid;
        $data['tipe']           = $tipe;        
        $data['totalattempt']   = $this->Assignment->getTotalAttempt($wsid, $npm);
        $data['totalfailed']    = $this->Assignment->getTotalFailed($wsid, $npm);
        $data['attempts']       = $this->Assignment->getAttempts($npm, $wsid);
        $data['latest']         = $this->Assignment->getLatestAtt($npm, $wsid);
    ...
    public function getAnswers($id_ws, $npm_mhs, $tipe, $created_at) {
        if($created_at != '404'){
            if ($tipe == 1) {
                ...
                $data = $this->Assignment->getAnswerMAT($id_ws, $npm_mhs, $created_at);
                $date_attempt = str_replace("%", " ", $created_at);
                if($data[0]->TGLFEEDBACK_WSMD){
                    echo '
                    <div class="row">
                ...
                $no = 1;
                foreach ($data as $items) {
                    $text = str_replace("<p>", "", $items->SOAL_MAT);
                    echo '
                        <div>
                            <span><b>'.$no.'. '.$text.'</b>Answer: '.$items->JAWABAN_MATR.'</span><br>
                            <span style="font-size:12px;color:#bbbbbb;"> Correct Answer : '.$items->KUNCIJAWABAN_MAT.'</span>
                        </div>
                        <br>
                        ';
                    $no++;
            ...
            echo '<span>No attempt has been made </span>';
        };        
    }
